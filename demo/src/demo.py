...
class CityListener(City.Listener):
    ...
    def on_map_added(self, map_obj):
        print(f"Map {map_obj.type()} added")

    def on_map_removed(self, map_obj):
        print(f"Map {map_obj.type()} removed")

    def on_path_added(self, path):
        print(f"Path {path.type()} added")

    def on_path_removed(self, path):
        print(f"Path {path.type()} removed")

    def on_unit_added(self, unit):
        print(f"Unit {unit.type()} {unit.m_id} added")

    def on_unit_removed(self, unit):
        print(f"Unit {unit.type()} {unit.m_id} removed")

    def on_agent_added(self, agent):
        print(f"Agent {agent.type()} added")

    def on_agent_removed(self, agent):
        print(f"Agent {agent.type()} removed")
...
class GlassBoxDemo:
    ...
    def init_demo_cities(self, simfile=None):
        ...
        # Use parsed types from simulation (matching C++ demo)
        ...
        # Get agent types from parsed script
...
def main():
    ...
    # Initialize demo
    demo = GlassBoxDemo(1024, 768, "OpenGlassBox Simulation - FIXED")
